...
const VerifyForm = () => {
  ...
  // Cập nhật email và mã xác nhận nếu có trong URL
  useEffect(() => {
  ...
  const onSubmit = async (data: VerifyReqType) => {
    ...
    router.push(`/login?email=${encodeURIComponent(data.email)}`); // Chuyển hướng đến login sau khi xác thực thành công
    } catch (error) {
