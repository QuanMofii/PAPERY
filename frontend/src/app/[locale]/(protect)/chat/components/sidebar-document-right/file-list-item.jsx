...
export function FileItem({ file }) {
    ...
    return (
        <div
            className={`relative flex h-full w-full items-center justify-between rounded-lg ${file.type === 'pdf' ? 'bg-orange-400' : 'bg-blue-500'} `}>
            <div
            ...
            <div className='flex flex-col gap-2 rounded-r-lg bg-gray-500 p-2 text-white'>
                <BookOpen className='h-4 w-4' />
                <FilePen className='h-4 w-4' />
                <Trash2 className='h-4 w-4' />
            </div>
            <div
                className={`absolute right-0 z-50 flex h-full w-[90%] justify-between gap-1 rounded-lg bg-yellow-400 px-2 ${open ? '-translate-x-6.5' : 'translate-x-0'} transition-all`}>
