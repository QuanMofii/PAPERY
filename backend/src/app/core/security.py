...
async def create_verification_token(email: str, token_type: TokenType) -> str:
    ...
    if token_type == TokenType.RESET_PASSWORD:
        expires = datetime.now(UTC) + timedelta(minutes=settings.RESET_PASSWORD_EXPIRE_MINUTES)
    else:  # VERIFY_ACCOUNT
        expires = datetime.now(UTC) + timedelta(minutes=settings.VERIFY_ACCOUNT_EXPIRE_MINUTES)
...
async def authenticate_user(username_or_email: str, password: str, db: AsyncSession) -> dict[str, Any] | Literal[False]:
    if "@" in username_or_email:
        db_user: Any = await crud_users.get(db=db, email=username_or_email, is_deleted=False)
    else:
